...
// functions/getLocation.ts
var import_node_fetch = __toModule(require_lib3());
exports.handler = async (event) => {
  let location = event.queryStringParameters.location;
  try {
    const response = await (0, import_node_fetch.default)(`https://api.mapbox.com/geocoding/v5/mapbox.places/${location}.json?access_token=${process.env.MAPBOX_ACCESS_TOKEN}`);
