...
// functions/getRandomImage.ts
var import_node_fetch = __toModule(require_lib3());
exports.handler = async (event) => {
  let location = event.queryStringParameters.location;
  console.log(location);
  try {
    const response = await (0, import_node_fetch.default)(`https://api.unsplash.com/photos/random?client_id=${process.env.UNSPLASH_ACCESS_KEY}&orientation=landscape&query=${location}`);
